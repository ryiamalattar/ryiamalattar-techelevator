...
public class Exercises {
	...
	public String makeOutWord(String out, String word) {
		String leftTags = out.substring(0,2);
		String rightTags = out.substring(2);
		
		return leftTags+word+rightTags;
	}
	...
	String lastTwo = str.substring(str.length()-2);
	String remain = str.substring(0,str.length()-2);
	
	String rotate = lastTwo + remain;
		return rotate;
	}
	...
	public String middleTwo(String str) {
		...
		String lastChar = firstHalf.substring(firstHalf.length()-1);
		String firstChar = secondHalf.substring(0,1);
		
		String midTwo = lastChar + firstChar;
	...
	public String nTwice(String str, int n) {
		...
		char[] lastCharArr = new char[n];
		
		for (int i=0; i< n ; i++) {
			firstCharArr[i] = charArray[i];
		}
		int j = 0;
		for (int i = charArray.length-n; i<charArray.length; i++) {
			lastCharArr[j] = charArray[i];
			j++;
				}
		String theFirstThree = new String(firstCharArr);
		String theLastThree = new String(lastCharArr);
		
		return theFirstThree+theLastThree;
	...
	public String middleThree(String str) {
		...
		String half = str.substring((str.length()/2)-1);
		String midThree = half.substring(0,3);
	...
	public String stringBits(String str) {
		...
		char[] charArray = str.toCharArray();
		char[] oddCharArray = new char[(str.length()/2)+1];
		char[] evenCharArray = new char[(str.length()/2)];
		...
		for (int i = 0; i< charArray.length ; i++) {
			...
			if (i % 2 == 0) {
				oddCharArray[j] = charArray[i];
				j++;
			}
		}
		
		String newStr = new String(oddCharArray);
		return newStr;
		}
			...
			for (int i = 0; i< charArray.length ; i++) {
				...
				if (i % 2 == 0) {
					evenCharArray[j] = charArray[i];
					j++;
				}
			}
			
			String newStr = new String(evenCharArray);
			return newStr;
		}
