...
select (gnp/population) as per_capita_GNP_US$ from country where continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
select (2020 - indepyear) as number_of_years_since_independance from country where indepyear is not null;