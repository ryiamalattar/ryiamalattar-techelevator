...
SELECT  CAST(AVG(lifeexpectancy) AS DECIMAL(10,2)) AS avg_lifeexpectancy, MIN(lifeexpectancy) AS min_lifeexpectancy, MAX(lifeexpectancy) AS max_lifeexpectancy, continent
FROM country
...
SELECT payment.*, customer.first_name, customer.last_name, customer.activebool as active_customer
FROM payment
JOIN customer ON payment.customer_id = customer.customer_id
WHERE payment_id = 16666
