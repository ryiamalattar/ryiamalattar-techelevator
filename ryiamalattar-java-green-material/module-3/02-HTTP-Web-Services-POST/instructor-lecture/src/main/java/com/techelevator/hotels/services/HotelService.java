...
public class HotelService {
	...
	public Reservation addReservation(String newReservation) {
		...
		HttpEntity<Reservation> entity = new HttpEntity(reservation, headers); // package of our data and header info

		Reservation reservationResult = null;
		try {
			reservationResult = restTemplate.postForObject(
					BASE_URL + "hotels/" + reservation.getHotelID() + "/reservations", entity, Reservation.class);
	 ...
	 * @return
	 */
	public Reservation updateReservation(String CSV) throws RestClientResponseException, ResourceAccessException{
	...
	public void deleteReservation(int id) {
		...
		try {
			...
			if (ex.getRawStatusCode() == 201) {
				System.out.println("You are not authorized dude!");
			}
		} catch (ResourceAccessException ex1) {
			System.out.println("" + ex1.getMessage());
		}
